...
function loadWidget(config) {
	...
	if (typeof cdnPath === "string") {
		useCDN = true;
		if (!cdnPath.endsWith("/")) cdnPath += "/";//自动补全斜杠
	} else if (typeof apiPath === "string") {
		if (!apiPath.endsWith("/")) apiPath += "/";//自动补全斜杠
	} else {
		console.error("文件接口无效！");
		return;
	}
	...
	let userAction = false,
		userActionTimer,
		messageTimer,
		messageArray = ["好久不见，日子过得好快呢……", "大坏蛋！你都多久没理人家了呀，嘤嘤嘤～", "嗨～快来逗我玩吧！", "拿小拳拳锤你胸口！", "记得时常温习所学知识哦"];
	window.addEventListener("mousemove", () => userAction = true);//人物目光是否跟随鼠标移动
	window.addEventListener("keydown", () => userAction = true);//鼠标点击
	setInterval(() => {
		if (userAction) {
			userAction = false;
	...
	(function registerEventListener() {
		// document.querySelector("#waifu-tool .fa-bell").addEventListener("click", showHitokoto);
		document.querySelector("#waifu-tool .fa-comment").addEventListener("click", () => {
				showMessage("我在认真聆听ing。", 2000, 11);
		...
		document.querySelector("#chatbox .x").addEventListener("click",() =>
		{
		...
		document.querySelector("#waifu-tool .fa-info-circle").addEventListener("click", () => {
			open("http://localhost:80/Live2D_widget/");
		});
	...
	(function welcomeMessage() {
		let text;
		if (location.pathname === "/HTML/demo.html") { // 如果是主页
	...
	async function loadModel(modelId, modelTexturesId, message) {
		...
		if (useCDN) {
			...
			// loadlive2d("live2d", `${cdnPath}model/ShizukuTalk/shizuku-48/index.json`);
